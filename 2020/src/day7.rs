    ...
    for line in lines {
        ...
        if !line.contains("contain no other bags") {
            for part in parts.get(1).unwrap().split(',') {
                let combo = part.trim().split(" bag").next().unwrap();
...
fn search<'a>(start: &'a str, rules: &HashMap<&str, Vec<(i32, &str)>>, mut seen: HashSet<&'a str>) -> bool {
    ...
    if seen.contains(start) || !rules.contains_key(start) || (rules.contains_key(start) && rules.get(start).unwrap().is_empty()) {
        return false;
    }
    seen.insert(start);
    rules.get(start).unwrap().iter().any(|s| search(s.1, rules, seen.clone()))

}

fn recurse(k: &str, rules: &HashMap<&str, Vec<(i32, &str)>>) -> i32 {
    if rules.contains_key(k) && rules.get(k).unwrap().is_empty() {
        return 0;
    }
    rules.get(k).unwrap().iter().map(|rule| rule.0 + rule.0 * recurse(rule.1, rules)).sum()
}

fn part1(lines: &[&str]) -> usize {
    let rules = parse(lines);
    rules.keys().map(|key| *key != "shiny gold" && search(key, &rules, HashSet::new())).filter(|x| *x).count()
}

fn part2(lines: &[&str]) -> i32 {
    let rules = parse(lines);
    recurse("shiny gold", &rules)
}
